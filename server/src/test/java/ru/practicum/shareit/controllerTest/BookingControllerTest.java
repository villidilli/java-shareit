...
public class BookingControllerTest {
    ...
    public void createBooking_thenReturnBooking() {
        when(bookingService.create(any(), any())).thenReturn(responseDto);
    ...
    public void createBooking_whenBookerNotFound_thenNotFoundExceptionThrow() {
        when(bookingService.create(any(), any())).thenThrow(new NotFoundException(USER_NOT_FOUND));
    ...
    public void getAllByBooker_whenFromAndSizeDefault_thenReturnListBookings() {
        ...
        mockMvc.perform(MockMvcRequestBuilders.get("/bookings/")
                        .contentType(MediaType.APPLICATION_JSON)
                        .header(PARAM_USER_ID, user.getId())
                        .param(PARAM_NAME_BOOKING_STATE, BookingState.ALL.name())
                        .param(FIRST_PAGE, "0")
                        .param(SIZE_VIEW, "999")
                        .accept(MediaType.APPLICATION_JSON)
                        .characterEncoding(StandardCharsets.UTF_8))
